...
<title>How every number on this site is computed</title><link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=IBM+Plex+Sans:ital,wght@0,400;0,500;0,600;1,400&family=Source+Serif+4:ital,opsz,wght@0,8..60,400;0,8..60,600;1,8..60,400&display=swap"><link rel="stylesheet" href="ise.css"></head><body><a class="skip" href="#method">Skip to the method</a><main id="method">''']
    ...
    o.append('<p class="blurb">It is a minimum and not a replacement, so a premise better established than the conclusion leaves the conclusion where its rows put it, and a premise stated in words that nobody has opened a page for caps nothing rather than capping at zero. This rule is why the five beliefs here read 0.50 while citing twenty findings between them. Their rows argue higher; each is held down by a necessary premise nobody has argued, which sits at 0.50 because nothing beneath it is established either way. That is the rule working, and it is also the thing to check first when a number looks wrong: each belief page says in words what is holding it and names the premise.</p>')
    ...
    o.append('<p class="form"><span class="lab">Media page</span> quality from its own argument rows, impact from its impact rows, each by the argued-truth rule above</p>')
    o.append('<p class="blurb">A work is scored on whether what it says holds up and, separately, on how much of the corpus it moved. The two are kept apart because a widely read work that is wrong and an unread work that is right are different problems.</p>')
