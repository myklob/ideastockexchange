-- Add columns and tables present in schema.prisma but missing from the migration chain.
-- These were added via db push during development and never captured as migrations.

-- Belief: related/supports links and falsifiability two-sided fields
ALTER TABLE "Belief" ADD COLUMN "relatedBeliefs" TEXT;
ALTER TABLE "Belief" ADD COLUMN "supportsBeliefs" TEXT;

-- Compromise: new sub-fields
...
-- InterestsAnalysis: primary pair columns
...
-- ValuesAnalysis: divergence evidence and shift condition
...
-- New tables for values/interests analysis
...
-- Argument: add claim, famousQuote, quoteAuthor, quoteAuthorUrl, argumentScore columns
-- (these require a table rewrite because of SQLite's ALTER TABLE limitations with indexes)
