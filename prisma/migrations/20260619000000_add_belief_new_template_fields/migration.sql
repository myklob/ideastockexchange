-- AlterTable Belief
-- Add new template fields: two-sided falsifiability, net interpretation,
-- and related/supports metadata headers.
...
-- AlterTable Argument
-- Add argument display fields: short label, famous quote, quote attribution,
-- and the argument's own score column.
...
-- Add measurement fields for the new template: how measured, current status, target threshold.
...
-- Add divergence evidence and shift-question fields from the new template.
...
-- Add Primary Conflict Pair fields for the new template interest section.
...
-- Add three-column compromise form fields from the new template.
...
CREATE TABLE "InterestEntry" (
    ...
    "evidenceBasis" TEXT,
    "connectedValue" TEXT,
    "pretextual" INTEGER NOT NULL DEFAULT 0,
    "sortOrder" INTEGER NOT NULL DEFAULT 0,
    "createdAt" DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
