-- AlterTable Belief: new template columns added after the last tracked migration.
...
-- AlterTable Argument: short label, quote, and score columns from the April 2026 template.
...
-- CreateTable ValueRanking: one row per value in the Shared Values / Different Rankings table.
...
-- CreateTable InterestEntry: one row per interest in the Supporters / Opponents interest tables.
...
-- CreateTable SharedInterest: one row per shared interest (foundation for compromise).
...
-- CreateTable DisputeType: Empirical / Definitional / Values row per belief.
