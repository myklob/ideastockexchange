-- Add all schema fields that were never captured in a prior migration.

-- Belief: two-sided falsifiability test
ALTER TABLE "Belief" ADD COLUMN "falsifiabilityConfirm" TEXT;
ALTER TABLE "Belief" ADD COLUMN "falsifiabilityFalsify" TEXT;

-- Belief: net score interpretation and related-belief metadata
...
-- Argument: label, quote, and score columns
...
-- ObjectiveCriteria: measurement and threshold columns
...
-- ValuesAnalysis: divergence evidence and shift prompt
...
-- Compromise: three-column template fields
...
-- New table: ValueRanking (Shared Values, Different Rankings)
...
-- New table: InterestEntry (Likely Interests of Supporters / Opponents)
...
-- New table: SharedInterest (foundation for compromise)
...
-- New table: DisputeType (Empirical / Definitional / Values)
