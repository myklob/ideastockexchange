-- AlterTable Belief: add two-sided Falsifiability Test fields, Net Belief Score
-- interpretation line, and header metadata. Added to schema without a migration
-- during the April 2026 template restructuring.
...
-- AlterTable Compromise: add new template fields for compromise detail.
...
-- AlterTable InterestsAnalysis: add primary pair fields for new interests template.
...
-- AlterTable ObjectiveCriteria: add how-to-measure, current status, and target fields.
...
-- AlterTable ValuesAnalysis: add divergence evidence and whatWouldShift fields.
...
-- AlterTable Argument: add argument-tree display fields and FK constraint for
-- importanceBeliefId. SQLite requires table recreation to add FK constraints.
...
-- CreateTable ValueRanking (new template: per-belief value ranking rows)
...
-- CreateTable InterestEntry (new template: Likely Interests of Supporters/Opponents)
...
-- CreateTable SharedInterest (new template: Shared Interests foundation for compromise)
...
-- CreateTable DisputeType (new template: Empirical / Definitional / Values rows)
