-- AlterTable Belief: add template v2 fields that were added to schema.prisma
...
-- AlterTable Argument: add display/label fields from the new argument-trees template.
...
-- AlterTable ObjectiveCriteria: add measurement/status/target columns.
...
-- CreateTable ValueRanking (Values & Interests section, new template)
...
-- CreateTable SharedInterest (Shared Interests / common ground, new template)
...
-- CreateTable DisputeType (Dispute Types table, new template)
...
-- AlterTable ValuesAnalysis: add advertised-vs-actual motivation evidence fields.
