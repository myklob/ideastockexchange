-- Sync schema drift: fields added to schema.prisma that were never migrated.
...
-- AlterTable: Argument (add columns if missing; SQLite has no IF NOT EXISTS for columns)
-- We must redeclare the table to safely add columns that may or may not exist.
...
INSERT INTO "new_Argument" ("beliefId", "createdAt", "depth", "id", "impactScore", "importanceBeliefId", "importanceScore", "linkageScore", "linkageScoreType", "linkageType", "parentBeliefId", "side", "updatedAt")
    SELECT "beliefId", "createdAt", "depth", "id", "impactScore", "importanceBeliefId", "importanceScore", "linkageScore", "linkageScoreType", "linkageType", "parentBeliefId", "side", "updatedAt" FROM "Argument";
DROP TABLE "Argument";
