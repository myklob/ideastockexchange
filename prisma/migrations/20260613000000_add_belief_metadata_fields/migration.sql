-- AlterTable Belief
-- Add two-sided Falsifiability Test fields, Net Belief Score interpretation,
-- and header metadata fields (relatedBeliefs, supportsBeliefs).
...
-- AlterTable Argument
-- Add short label, famous quote, author attribution, and per-argument score columns.
...
-- Add divergence-evidence columns and "what would shift" prompt field (new template).
...
-- Add Best Compromise Solutions three-column fields (new template).
...
-- Add "how to measure", current status, and target/threshold columns (new template).
