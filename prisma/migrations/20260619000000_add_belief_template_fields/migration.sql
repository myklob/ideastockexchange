-- AlterTable Belief
-- Add fields for the two-sided Falsifiability Test, net score interpretation,
-- and header metadata (related/parent beliefs).
...
-- AlterTable Argument
-- Add display fields for the argument trees cell: short claim label, famous
-- quote, quote attribution, and the argument's own displayed score.
...
-- Add three-column Best Compromise Solutions fields (new template).
...
-- Add columns for how the criterion is measured, its current status, and target.
...
-- Add divergence evidence columns and the "what would shift" prompt answer.
...
-- One row per value in the Shared Values, Different Rankings table (new template).
...
-- One row per interest in the Likely Interests tables (new template).
...
-- One row per shared interest in the foundation-for-compromise table (new template).
...
-- One row per dispute type (Empirical / Definitional / Values) in the new template.
