-- Add columns that were added to the schema via `prisma db push` (no migration was
-- generated). Without these, the integration tests build a fresh DB from the migration
-- chain and then hit "column does not exist" errors from the Prisma client, which was
-- generated from the full schema.

-- Belief: new template fields (falsifiability test, Net Belief Score interpretation,
-- header metadata for related beliefs and beliefs-this-supports).
...
-- Argument: new template fields (short label, famous quote, submitter, score column).
...
-- ObjectiveCriteria: new template columns (how measured, current status, target).
...
-- CreateTable: ValueRanking — Shared Values, Different Rankings table (new template).
...
-- CreateTable: InterestEntry — Likely Interests of Supporters / Opponents (new template).
...
-- CreateTable: SharedInterest — foundation for compromise (new template).
...
-- CreateTable: DisputeType — Empirical / Definitional / Values dispute types (new template).
...
-- ValuesAnalysis: new template columns (divergence evidence, what-would-shift).
...
-- Compromise: Best Compromise Solutions three-column form (new template).
