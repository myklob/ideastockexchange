-- Add missing Belief columns that were applied via db:push but never captured in a migration.
...
-- Add missing Argument columns (label / quote / score fields added after the last table recreation).
...
-- Create missing tables that were added to the schema via db:push without migrations.
