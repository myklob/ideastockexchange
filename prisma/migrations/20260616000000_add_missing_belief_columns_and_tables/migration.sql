-- Add columns to ObjectiveCriteria that were added in schema but never migrated
...
-- Add columns to ValuesAnalysis that were added in schema but never migrated
...
CREATE UNIQUE INDEX IF NOT EXISTS "ValuesAnalysis_beliefId_key" ON "ValuesAnalysis"("beliefId");

-- Add columns to InterestsAnalysis that were added in schema but never migrated
...
CREATE UNIQUE INDEX IF NOT EXISTS "InterestsAnalysis_beliefId_key" ON "InterestsAnalysis"("beliefId");

-- Add columns to Compromise that were added in schema but never migrated
...
-- Add columns to Argument that were added in schema but never migrated
...
-- Add columns to Belief that were added in schema but never migrated
...
-- ValueRanking: one row per value in Supporters vs Opponents value table
...
-- InterestEntry: one row per interest in Supporters / Opponents interests table
...
-- SharedInterest: one row per shared interest (foundation for compromise)
...
-- DisputeType: one row per dispute classification (Empirical / Definitional / Values)
