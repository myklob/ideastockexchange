-- Add new Belief columns from belief-analysis template v2 (April 2026).
...
-- Add new Argument display columns and fix importanceBeliefId FK constraint (template v2).
-- SQLite requires a full table rewrite to add FK constraints, so we recreate the table.
...
-- Add new Compromise columns (template v2).
...
-- Add new ObjectiveCriteria columns (template v2).
...
-- Add new ValuesAnalysis columns (template v2).
...
-- Add new InterestsAnalysis columns (template v2).
