...
- **zoning** — the argument tree of the worked zoning example
  (`../ise-zoning/`): belief z1 with four reasons a side, their own
  sub-reasons, and the real linkage sub-debates on the eight root edges. At
  m = 1 the root scores 0.5333 (z2/z3/z8 → 1, z7 → -1); at the default
  m = 0.7, 0.3733. Two root edges have a negative linkage score: both sides
  accept the reason and argue about what it bears on.
...
- `belief.php?id=z1` — one conclusion's page: two-column reasons, per-edge
...
- `src/lib/ise-pages/` and `examples/ise-zoning/` — the current page/edge
  model, where the same zoning belief is worked all the way down and every
  multiplier is an argued page rather than a count.
