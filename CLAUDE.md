...
- Real algorithm routes: `/algorithms/linkage-scores`, `/algorithms/reason-rank`, `/algorithms/objective-criteria`, `/algorithms/strong-to-weak`, `/algorithms/belief-equivalency`, `/algorithms/combine-similar-beliefs`. If you add a link to a belief page, verify the target exists in this list or `src/app/` — or use plain text (Rule 5). The old `/Linkage%20Scores`, `/ReasonRank`, `/Argument%20scores%20from%20sub-argument%20scores` etc. were dead wiki links; they are now plain text in the components.
