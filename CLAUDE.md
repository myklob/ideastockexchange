...
- `/leverage` — the work queue: every unsettled argument edge in the corpus ranked by Decision Leverage, plus the per-belief roll-up and the beliefs resting on one crux. JSON twin at `/api/leverage`.
