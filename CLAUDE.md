...
- **Prisma 7** with SQLite via `@prisma/adapter-better-sqlite3`. Schema lives in `prisma/schema.prisma`. Generated client lives at `src/generated/prisma/` and is committed to git. After changing the schema, re-run `npm run db:generate` to regenerate it.
