...
- **Layer 1 — Justification (internal denominator).** `J(X) = (Pro − Con) / (Pro + Con)`,
  ...
  net-refutation trigger (negative ⇒ net-refuted). Identity: `J = 2p − 1` where
...
first and state the shift so nobody mistakes it for a probability. Stating beliefs in
positive form (invert negative claims before scoring) reinforces this — a class of
positive claims mostly avoids the signed-score problem at the top level.
