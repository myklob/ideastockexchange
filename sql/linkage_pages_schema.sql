...
CREATE TABLE IF NOT EXISTS `nodes` (
  ...
  INDEX `idx_score` (`argument_score` DESC)
);
...
CREATE TABLE IF NOT EXISTS `linkages` (
  ...
  INDEX `idx_score` (`linkage_score` DESC),
  ...
  CONSTRAINT `chk_linkage_score_range` CHECK (`linkage_score` IS NULL OR (`linkage_score` >= 0 AND `linkage_score` <= 1))
);
