...
async function payOut(tx: Tx, contract: MarketContract, outcome: 'YES' | 'NO') {
  // Cancel resting orders first, refunding buy-side escrow. Balances move by
  // increment, not by writing back a snapshot: one user can hold several
  // resting orders on a contract, and every row here carries the same balance
  // read, so writing snapshot+refund would keep only the last refund.
