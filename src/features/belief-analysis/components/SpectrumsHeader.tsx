...
interface SpectrumsHeaderProps {
  /** Positivity / valence: -100 (extremely negative) to +100 (extremely positive). */
  positivity: number
  /** Specificity: 0 (highly general principle) to 1 (highly specific case). */
  specificity: number
  /** Claim strength / intensity: 0 (weak) to 1 (extreme). */
...
/**
 * The compact three-axis coordinate display for a belief, surfaced as part of the
 * page header metadata (not a canonical section). Shows where the belief sits on:
 *   Valence (Spectrum A): negative ↔ positive
 *   Specificity (Spectrum B): general principle ↔ specific case
 *   Intensity (Spectrum C): weak claim ↔ extreme claim
 *
 * Defined in the &ldquo;Three Spectrums&rdquo; section of /One Page Per Belief.
 */
export default function SpectrumsHeader({ positivity, specificity, claimStrength }: SpectrumsHeaderProps) {
  ...
  return (
    <div className="border border-[var(--border)] rounded p-3 mb-8 bg-gray-50">
      <div className="text-xs font-semibold text-[var(--muted-foreground)] mb-2 flex items-center gap-2">
        ...
        <span className="text-[10px] font-normal text-[var(--muted-foreground)]">(what is this?)</span>
      </div>
