...
const MEDIA_TYPE_GLYPHS: Record<string, string> = {
  image: '\u{1F4F7} Photo',
  scientific_paper: '\u{1F4C4} Scientific Paper',
  article: '\u{1F4F0} Article',
  song: '\u{1F3AD} Meme / Cartoon',
  poem: '\u{1F3AD} Meme / Cartoon',
  movie: '\u{1F3A5} Video / Documentary',
  podcast: '\u{1F3A5} Video / Documentary',
  book: '\u{1F4DA} Book',
}

function mediaTypeGlyph(mediaType: string): string {
  return MEDIA_TYPE_GLYPHS[mediaType] ?? `\u{1F4C4} ${mediaType.replace(/_/g, ' ')}`
}
    ...
    <section>
      ...
      <div className="mb-8">
        <h3 className="text-base font-semibold mb-2 flex items-center gap-2">
          <span>\u{1F4C4}</span> Text and Data Sources
        </h3>
      ...
      <div>
        <h3 className="text-base font-semibold mb-2 flex items-center gap-2">
          <span>\u{1F4F8}</span> Visual and Video Evidence
        </h3>
        <div className="overflow-x-auto">
          <table className="w-full border-collapse border border-gray-300 text-sm">
            <thead>
