 ...
 * page only (no minting stub pages), short transcripts, per-client and
 * global daily limits.
 */
export async function POST(request: Request) {
  ...
  // must not mint pages, so the page has to exist already.
  ...
  let messages: ConversationMessageInput[]
  if (typeof body.transcript === 'string') {
    ...
    messages = parseTranscript(body.transcript)
  } else if (Array.isArray(body.messages)) {
    messages = body.messages as ConversationMessageInput[]
  } else {
    return agentJson({ error: 'Provide a transcript string ("Author: message" per line) or a messages array.' }, { status: 422 })
  }
