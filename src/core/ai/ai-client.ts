...
export function createAIClient(configPath?: string): AIClient {
  ...
  if (configPath) {
    try {
      ...
      if (config !== null && typeof config === 'object' && 'llm' in config) {
        const llm = (config as Record<string, Record<string, unknown>>).llm;
        return new AIClient({
          provider: (llm.provider as AIProviderConfig['provider']) || defaultConfig.provider,
          apiBase: (llm.api_base as string) || defaultConfig.apiBase,
          model: (llm.model as string) || defaultConfig.model,
          apiKey: llm.api_key as string | undefined,
          temperature: (llm.temperature as number) || defaultConfig.temperature,
          maxTokens: (llm.max_tokens as number) || defaultConfig.maxTokens,
          timeout: (llm.timeout as number) || defaultConfig.timeout,
        });
      }
    } catch (error) {
